...
<c:choose>
		...
		<div class="row">
			...
			<c:forEach var="list" items="${history.content}">
				<div class="row mb-4" id="order${list.orderId}">
					...
					<div class="col-5">
						...
						<a class="btn btn-primary"
							href="/store/manager/history/success/${list.orderId}">Giao</a>
			...
			<div class="col-9 offset-3">
				<c:if test="${history.number>1}">
					<a href="/store/manager/history/order/ship?page=1"
						class="btn btn-primary">1</a>
				</c:if>
				<c:if test="${history.number>0}">
					<a
						href="/store/manager/history/order/ship?page=${history.number <= 0 ? 1 : history.number}"
				...
				<c:if test="${(history.totalPages - history.number-1) > 0}">
					<a
						href="/store/manager/history/order/ship?page=${history.number + 2 >= history.totalPages ? history.totalPages :  history.number + 2}"
				...
				<c:if test="${(history.totalPages - history.number-1) > 1}">
					<a
						href="/store/manager/history/order/ship?page=${history.totalPages}"
	...
	<c:otherwise>
		<div class="row">
			<h2 class="col-8">Chưa có đơn hàng nào đang được giao cả</h2>
		</div>
...
<script>
	function cancel(id) {
		...
		img.setAttribute('action', '/store/manager/history/ship/cancel/' + id);
	}
</script>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h1 class="modal-title fs-5" id="staticBackdropLabel">Lý do không nhận đơn hàng</h1>
