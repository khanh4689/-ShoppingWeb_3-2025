...
<c:choose>
		...
		<div class="row">
				...
				<div class="row mb-4" id="order${list.orderId}">
					...
					<div class="col-5">
						<button class="btn btn-primary" type="button"
							...
							Tình trạng</button>
						<a class="btn btn-primary"
							href="/store/manager/history/ship/${list.orderId}">Ship</a>
						<button class="btn btn-primary" type="button"
			...
			<div class="col-9 offset-3">
				<c:if test="${history.number>1}">
					<a href="/store/manager/history/order/confirm?page=1"
						class="btn btn-primary">1</a>
				</c:if>
				<c:if test="${history.number>0}">
					<a
						href="/store/manager/history/order/confirm?page=${history.number <= 0 ? 1 : history.number}"
				...
				<c:if test="${(history.totalPages - history.number-1) > 0}">
					<a
						href="/store/manager/history/order/confirm?page=${history.number + 2 >= history.totalPages ? history.totalPages :  history.number + 2}"
				...
				<c:if test="${(history.totalPages - history.number-1) > 1}">
					<a
						href="/store/manager/history/order/confirm?page=${history.totalPages}"
	...
	<c:otherwise>
		<div class="row">
			<h2 class="col-8">Chưa có đơn nào đang được xác nhận.</h2>
		</div>
